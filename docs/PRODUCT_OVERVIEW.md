...
- **Design** builds a "craft" - a flow, screen, component, or pattern
  prototype - using real NDS components and mock data, instead of static
  Figma frames.
...
### 1. Craft gallery & navigation
- **Home gallery** (`/`) lists every craft as a card: title, description,
  tags, and status (`idea` / `wip` / `review` / `ready`).
- **Top nav craft switcher** - jump between crafts from anywhere.
- **⌘K command palette** - fuzzy-search crafts by name or tag.
- **Light/dark theme toggle**, applied instantly across all NDS tokens.

### 2. Variant comparison
Any craft can register multiple **variants** (alternative flows or
...
- All markers for a craft collect into a **Notes panel** (side sheet) -
...
For crafts that span multiple screens or need a spatial layout, a craft can
opt into a large pan/zoom canvas instead of a single page:
...
### 6. Live crafts today
| Craft | Status | What it shows |
|---|---|---|
| **Employee directory - filters** (`/crafts/employee-filters`) | WIP | Full Figma Web shell + nested filter menu + editable filter chips + AI prompt filtering + Display customization + employee table, shown as three draggable canvas windows. |
| **Playground demo** (`/crafts/demo-annotations`) | Ready | Reference craft for variants, annotations, and the notes panel - the template designers copy to start a new one. |
...
- **Stakeholder share mode** - a read-only, link-based view of a craft
...
- **Guided walkthrough / presentation mode** - step through a craft's
...
- **Figma round-trip** - push a craft (or a specific variant) back into
...
- **Component gallery** - a Storybook-style catalog of every NDS primitive in
  isolation (all states/variants), separate from full crafts, for
...
- **Craft history/versioning** - keep prior variants of a craft around
  after a decision is made, so "what did we consider and reject" isn't
  lost when the registry entry is edited.
- **Tagging/status filters on the gallery** - filter the home gallery by
  status or tag once the number of crafts grows past a handful.
- **Lightweight usability signal** - optional click/interaction tracking
  scoped to a single craft session, to get quick directional signal in
