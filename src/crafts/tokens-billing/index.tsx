...
/**
 * Tokens & billing - Settings > AI (Tokens, Usage) and Settings > Billing
 * (Payment methods, Credit), the navbar token pill + notifications, the
 * buy-tokens flow, and auto-reload. Same canvas/explorer/Live-App
 * architecture as every other craft; ported from a hand-built HTML
 * workbench that already resolved the pricing math and 18 balance presets.
 */
  ...
  const windows: Array<CanvasWindowDef> = [
    {
      ...
      url: 'nexus.niural.com/settings/ai/tokens',
      x: MARGIN_X,
      y: MARGIN_Y,
      width: FRAME_W,
      height: FRAME_H,
  ...
  for (const group of SCENARIOS) {
    ...
    for (const screen of group.screens) {
      windows.push({
        id: screen.id,
        title: screen.label,
        url: `nexus.niural.com/settings#${screen.id.toLowerCase()}`,
        x,
        y,
        width: FRAME_W,
        height: FRAME_H,
          ...
          extraActions={
            <div className="flex items-center gap-1">
              <Button
                ...
                title="Reset app data - clears the live App back to healthy defaults"
...
export function TokensBilling() {
  return (
    <ExplorerProvider catalog={EXPLORER_CATALOG} storageKey="tokens-billing">
      <TokensBillingCanvas />
    </ExplorerProvider>
  )
}
