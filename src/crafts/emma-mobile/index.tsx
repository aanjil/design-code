...
import { phoneFrameSize } from '@/components/playground/canvas'
...
/**
 * Emma Mobile - flow & state explorer, same architecture as multi-payroll
 * (Figma-style groups of frames, App = one live-interactive instance,
 * A-B catalog = a frozen review matrix) - the one new capability is that
 * every frame here is `chrome: 'phone'` sized to the iPhone 16 Pro Max's
 * own 440x956pt viewport (see canvas.tsx's phoneFrameSize/PHONE_BEZEL),
 * instead of the 1512x910 desktop browser frame every other craft uses.
 */

const { width: FRAME_W, height: FRAME_H } = phoneFrameSize(440, 956)
const FRAME_GAP = 56
const MARGIN_X = 100
const MARGIN_Y = 120
const GROUP_GAP = 160
  ...
  for (const group of SCENARIOS) {
    ...
    for (const screen of group.screens) {
      windows.push({
        id: screen.id,
        title: screen.label,
        url: `niural.app/emma#${screen.id.toLowerCase()}`,
        x,
        y,
        width: FRAME_W,
        height: FRAME_H,
          ...
          extraActions={
            <div className="flex items-center gap-1">
              <Button
                ...
                title="Reset app data - clears the live App back to signed out"
...
export function EmmaMobile() {
  return (
    <ExplorerProvider catalog={EXPLORER_CATALOG} storageKey="emma-mobile">
      <EmmaMobileCanvas />
    </ExplorerProvider>
  )
}
