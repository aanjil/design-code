...
/**
 * Scheduled Report Generation - flow & state explorer (PRD: Niural
 * Insights, NPE-9923). Same shape as multi-payroll: "App" is one real,
 * live-interactive instance of the whole feature (its own group, pinned
 * first, Play button on canvas + panel) backed by a persisted store
 * (live-store.ts) - separate from the deterministic A-D catalog below
 * it, which stays a frozen-state review matrix.
 */
  ...
  const windows: Array<CanvasWindowDef> = [
    {
      ...
      url: 'nexus.niural.com/insights/reports',
      x: MARGIN_X,
      y: MARGIN_Y,
      width: FRAME_W,
      height: FRAME_H,
  ...
  for (const group of SCENARIOS) {
    ...
    for (const screen of group.screens) {
      windows.push({
        id: screen.id,
        title: screen.label,
        url: `nexus.niural.com/insights/reports#${screen.id.toLowerCase()}`,
        x,
        y,
        width: FRAME_W,
        height: FRAME_H,
...
export function ScheduleReport() {
  return (
    <ExplorerProvider catalog={EXPLORER_CATALOG} storageKey="schedule-report">
      <ScheduleReportCanvas />
    </ExplorerProvider>
  )
}
