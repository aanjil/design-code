...
- Prototypes ("crafts") are registered in `src/crafts/registry.ts`, live in
  `src/crafts/<slug>/`, and mount via a thin route `src/routes/crafts/<slug>.tsx`
  inside `<CraftFrame>` (variant tabs + annotation markers + notes panel -
  see DESIGN.md → Playground structure). A craft covers whatever needs
  prototyping - a UX flow, a component, a pattern, or a full feature.
- The shell (`src/components/playground/`) provides the top nav, craft
  switcher, ⌘K palette, and theme toggle. It renders from the registry - new
  crafts appear everywhere automatically.
- Crafts are presentation artifacts for stakeholders: prefer visible design
  rationale (`<Annotate>` notes, variant comparisons) over app realism.
...
- Route files must exist before `src/crafts/registry.ts` can reference
...
- Build real crafts (e.g. table filters) on this structure - the playground
  demo (`/crafts/demo-annotations`) is the copyable reference.
